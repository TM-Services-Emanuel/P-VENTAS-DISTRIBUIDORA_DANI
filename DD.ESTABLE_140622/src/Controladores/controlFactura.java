...
public class controlFactura {
    ...
    public static void insertar(String cod, String dependencia, String iddep, String codB, String desc, String cant, String prec, String total, int iva, int montoCosto, JTable tabla) {
    ...
    public static void addmismoItemFactura(int fila, double cantTabla, int iva, double cantAdd, int precio, int costo/*ADD*/) {
        try {
            ...
            if (dependecia.equals("S")) {
                ...
                if ((StockT + Double.parseDouble(dlgVentas.txtCant.getText().trim())) > StockDepencencia) {
                    ...
                    if (array.busca(dfac.getCodArticulo()) != -1) {
                        ...
                        addmismoItemFactura(Nfila, cantTabla, iva, cant, precio, costo);
                        ...
                        if (cant < 0) {
                            ...
                            array.agregar(dfac);
                            insertar(String.valueOf(codA), dependencia, iddep, codB, desc, String.valueOf(cant), String.valueOf(precio), String.valueOf(monto), iva, montocosto, tabla);
                            String total = String.valueOf(getTotal());
                            String totalCosto = String.valueOf(getTotalCosto());
                            String exentas = String.valueOf(getExcetas());
                            String iva5 = String.valueOf(get5());
    ...
    {
        List lista;
        lista = GestionarFactura.listFacturasMoviles();
        for (int i = 1; i < lista.size(); i++) {
            DefaultTableModel tb = (DefaultTableModel) tabla.getModel();
            Object[] fila = (Object[]) lista.get(i);
            fila[0].toString();
